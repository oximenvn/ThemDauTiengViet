...
def remove_tone_file(in_path, out_path):
    with codecs.open(in_path, 'r', encoding='utf-8') as in_file,\
        ...
        for line in in_file:
            ...
            utf8_sen = re.split(';|:|\?|,|\.',utf8_line)
            for a_sen in utf8_sen:
                old_cut_pos=0
                max_len=200
                for i in range(0, len(a_sen), max_len):
                    try:
                        cut_pos = a_sen[old_cut_pos:].find(" ",old_cut_pos+max_len)
                        if cut_pos != -1 :
                            one = a_sen[old_cut_pos:cut_pos]
                            out_file.write(one.replace("\n", "")+"\t"+remove_tone_line(one).replace("\n", "")+"\n")
                            #print cut_pos
                            # print one
                            old_cut_pos = cut_pos
                        else :
                            # print len(a_sen[old_cut_pos:])
                            if len(a_sen[old_cut_pos:]) <255:
                                out_file.write(a_sen[old_cut_pos:].replace("\n", "")+"\t"+remove_tone_line(a_sen[old_cut_pos:]).replace("\n", "")+"\n")
                    except UnicodeDecodeError as e:
                        print 'Line with decode error:'
                        print e
                        print a_sen[old_cut_pos:cut_pos]+"\n"
                        print a_sen[old_cut_pos:]

def remove_tone_file_word(in_path, out_path):
    with codecs.open(in_path, 'r', encoding='utf-8') as in_file,\
            codecs.open(out_path, 'w', encoding='utf-8') as out_file:
        for line in in_file:
            utf8_line = line.encode('utf-8')
...
if __name__ == '__main__':
    #remove_tone_file('./data/VNESEcorpus.txt', './data/no_tone_VQ.txt')
    # remove_tone_file_word('./data/demo_test-all.txt', './data/all_4.txt')
    # remove_tone_file_word('./data/demo_test.txt', './data/no_tone_test.txt')
    decompose_predicted_test_file('./data/5.txt')
